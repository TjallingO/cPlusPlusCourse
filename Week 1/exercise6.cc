...
int main()
{
  ...
  // The modulo operator, using the number 2, checks whether a value is fully 
  // divisble (i.e. no remainder) by 2, and if so, returns 1 as the remainder 
  // (true), else 0 (false).
  ...
  // For integral divisions, any fractional part is discarded. Hence, any number 
  // divisible by two will equal the initial value when multiplied by two again,
  // others won´t.

  cout << (((value >> 1) << 1) == value ? "odd" : "even") << '\n';
  // The right shift operator will truncate the right-most digit of a binary
  // number.Shifting this resulting number to the left and adding a zero at 
  // the end (i.e. left shift) will return the original value once more only 
  // if the last digit was a zero to begin with; hence, it was even.
